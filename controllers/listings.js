...
export const createCsv = async (req, res) => {
  const { address, pageSize } = req.body;
  ...
  try {
    ...
    // API endpoint to fetch listing data
    const response = await axios.post(url, payloadData, { headers });

    const listings = response.data.data.results.listings;

    const startDate = DateTime.now();

    const endDate = DateTime.fromISO("2023-05-20");

    const dates = getDatesInRange(startDate, endDate);

    //creating csv template
    const csvWriter = createObjectCsvWriter({
      path: "listingData.csv",
      header: [
        { id: "listingId", title: "Listing ID" },
        { id: "headline", title: "Name" },
    ...
    const records = listings.map((listing) => {
      ...
      dates.forEach((date, index) => {
        record[date] =
          listing.rateSummary.rentNights !== null
            ? listing.rateSummary.rentNights[index] || ""
            : 0;
      });
      return record;
    });
    ...
    res.status(200).send({ message: "Listing data saved successfully" });
  } catch (error) {
    res.status(500).send({ error: "Failed to fetch and save listing data" });
  }
};
